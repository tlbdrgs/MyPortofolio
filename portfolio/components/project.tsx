...
export default function Project({ title, description, tags, imageUrl }: ProjectProps) {
    ...
    return <motion.div ref={ref} style={{
        ...
        <section className=" bg-gray-100 max-w-[42rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem]  hover:bg-gray-200 transition sm:group-even:pl-8 dark:bg-white/10 dark:hover:bg-white/20 dark:text-white">
            ...
            <Image className="absolute hidden sm:block top-8 -right-40 w-[25rem] rounded-lg shadow-2xl transition group-hover:scale-105
         group-hover:-translate-x-3 group-hover:translate-y-3 group-hover:-rotate-2
         group-even:group-hover:translate-x-3  group-even:group-hover:translate-y-3  group-even:group-hover:rotate-2
           group-even:-right-[initial] group-even:-left-40" src={imageUrl} alt={title} />
        </section>
    </motion.div>
}
